...
#include "ui_print_assig_sexage_by_list_wnd.h"

print_assig_sexage_by_list_wnd::print_assig_sexage_by_list_wnd(QSqlDatabase &db, s_data_app &data_app, QSettings &settings, QWidget *parent) :
    db(db),
    data_app(data_app),
    settings(settings),
    QDialog(parent),
    ui(new Ui::print_assig_sexage_by_list_wnd)
{
    ui->setupUi(this);
...
print_assig_sexage_by_list_wnd::~print_assig_sexage_by_list_wnd() {
    delete ui;
}

void print_assig_sexage_by_list_wnd::on_bn_cansel_clicked() {
    reject();
}

void print_assig_sexage_by_list_wnd::on_bn_date_assig_clicked() {
    ui->date_assig->setDate(QDate::currentDate());
}

QString print_assig_sexage_by_list_wnd::month_to_str(QDate date) {
    QString res;
    switch (date.month()) {
    case 1:
        res += "января ";
...
void print_assig_sexage_by_list_wnd::on_bn_edit_clicked(bool checked) {
    if (checked) {
...
void print_assig_sexage_by_list_wnd::on_bn_print_clicked() {
    ...
    bool res_medorgs = mySQL.exec(this, sql_medorgs, "Запросим персону, прикреплённую к данной МО", *query_medorgs, true, db, data_app);

    if (!res_medorgs || query_medorgs->size()<=0) {
        QMessageBox::warning(this, "Список не получен", "При выборе списка ЛПУ произошла неожиданная ошибка.");
    ...
    sql += "  left join (select aa.id_person, max(aa.id) as id "
                        "  from persons_assig aa "
                        " where aa.assig_date<='#DATE_REP#' "
                        "   and (aa.unsig_date is NULL or aa.unsig_date>'#DATE_REP#') "
                        " group by aa.id_person) qa on(qa.id_person=e.id) ";
    if (ui->ch_assig->isChecked())  sql+=" join ";
    else                            sql+=" left join ";
    sql += "    persons_assig a on(a.id=qa.id) "
    ...
    if (!res || query->size()<=0) {
        QMessageBox::warning(this, "Список не получен", "При выборе полного списка прикреплённых к ЛПУ произошла неожиданная ошибка.");
        delete query;
        this->setCursor(Qt::ArrowCursor);
        ui->bn_print->setEnabled(true);
        return;
    ...
    QString fname_res = data_app.path_out + "_REPORTS_/" +
            QDate::currentDate().toString("yyyy-MM-dd") + "  -  " + data_app.smo_short + " - " + data_app.filial_name + "  -  assig_sexage_tab - " + date_assig.toString("yyyy-MM-dd") + ".ods";
...
void print_assig_sexage_by_list_wnd::on_ln_smo_name_textChanged(const QString &arg1) {
    data_app.smo_name = ui->ln_smo_name->text();
}

void print_assig_sexage_by_list_wnd::on_ln_filial_director_textChanged(const QString &arg1) {
    data_app.filial_director = ui->ln_filial_director->text();
}

void print_assig_sexage_by_list_wnd::on_ln_filial_director_2_textChanged(const QString &arg1) {
    data_app.filial_director_2 = ui->ln_filial_director_2->text();
}

void print_assig_sexage_by_list_wnd::on_ln_filial_chif_textChanged(const QString &arg1) {
    data_app.filial_chif = ui->ln_filial_chif->text();
}

void print_assig_sexage_by_list_wnd::on_ln_terr_name_textChanged(const QString &arg1) {
    data_app.terr_name = ui->ln_terr_name->text();
}

void print_assig_sexage_by_list_wnd::on_ln_terr_name_rp_textChanged(const QString &arg1) {
    data_app.terr_name_rp = ui->ln_terr_name_rp->text();
}

void print_assig_sexage_by_list_wnd::on_bn_date_cur_clicked() {
    ui->date_assig->setDate(QDate::currentDate().addDays(-1));
}

void print_assig_sexage_by_list_wnd::on_bn_first_day_clicked() {
