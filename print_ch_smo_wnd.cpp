...
#include "ui_print_ch_smo_wnd.h"

print_ch_smo_wnd::print_ch_smo_wnd(QSqlDatabase &db, s_data_app &data_app, s_data_pers &data_pers, s_data_vizit &data_vizit, s_data_polis &data_polis, QSettings &settings, QWidget *parent)
 : db(db), data_app(data_app), data_pers(data_pers), data_vizit(data_vizit), data_polis(data_polis), QDialog(parent), settings(settings), ui(new Ui::print_ch_smo_wnd)
{
    ui->setupUi(this);
...
print_ch_smo_wnd::~print_ch_smo_wnd() {
    delete ui;
}

void print_ch_smo_wnd::on_bn_close_clicked() {
    reject();
}

QString print_ch_smo_wnd::date_to_str(QDate date) {
...
void print_ch_smo_wnd::on_bn_print_clicked() {
    ...
    QString rep_template = rep_folder + "req_ch_smo_new.odt";
    ...
    s_content = s_content.
        ...
        replace("#SMO_ORGN#", data_app.smo_ogrn).
        replace("#OPERATOR#", data_app.point_signer);

    s_content = s_content.
        replace("#R1#", ( (data_vizit.r_smo==1) ? "V" : " ") ).
        replace("#R2#", ( (data_vizit.r_smo==2) ? "V" : " ") ).
        replace("#R3#", ( (data_vizit.r_smo==3) ? "V" : " ") ).
        replace("#R4#", ( (data_vizit.r_smo==4) ? "V" : " ") ).
    ...
    s_content = s_content.
        replace("#1#", ( (data_polis.enp.isEmpty() || data_polis.enp.trimmed().length()< 1) ? QString(" ") : QString(data_polis.enp.trimmed().at( 0)) ) ).
        replace("#2#", ( (data_polis.enp.isEmpty() || data_polis.enp.trimmed().length()< 2) ? QString(" ") : QString(data_polis.enp.trimmed().at( 1)) ) ).
        replace("#3#", ( (data_polis.enp.isEmpty() || data_polis.enp.trimmed().length()< 3) ? QString(" ") : QString(data_polis.enp.trimmed().at( 2)) ) ).
        replace("#4#", ( (data_polis.enp.isEmpty() || data_polis.enp.trimmed().length()< 4) ? QString(" ") : QString(data_polis.enp.trimmed().at( 3)) ) ).
        replace("#5#", ( (data_polis.enp.isEmpty() || data_polis.enp.trimmed().length()< 5) ? QString(" ") : QString(data_polis.enp.trimmed().at( 4)) ) ).
        replace("#6#", ( (data_polis.enp.isEmpty() || data_polis.enp.trimmed().length()< 6) ? QString(" ") : QString(data_polis.enp.trimmed().at( 5)) ) ).
        replace("#7#", ( (data_polis.enp.isEmpty() || data_polis.enp.trimmed().length()< 7) ? QString(" ") : QString(data_polis.enp.trimmed().at( 6)) ) ).
        replace("#8#", ( (data_polis.enp.isEmpty() || data_polis.enp.trimmed().length()< 8) ? QString(" ") : QString(data_polis.enp.trimmed().at( 7)) ) ).
        replace("#9#", ( (data_polis.enp.isEmpty() || data_polis.enp.trimmed().length()< 9) ? QString(" ") : QString(data_polis.enp.trimmed().at( 8)) ) ).
        replace("#10", ( (data_polis.enp.isEmpty() || data_polis.enp.trimmed().length()<10) ? QString(" ") : QString(data_polis.enp.trimmed().at( 9)) ) ).
        replace("#11", ( (data_polis.enp.isEmpty() || data_polis.enp.trimmed().length()<11) ? QString(" ") : QString(data_polis.enp.trimmed().at(10)) ) ).
        replace("#12", ( (data_polis.enp.isEmpty() || data_polis.enp.trimmed().length()<12) ? QString(" ") : QString(data_polis.enp.trimmed().at(11)) ) ).
        replace("#13", ( (data_polis.enp.isEmpty() || data_polis.enp.trimmed().length()<13) ? QString(" ") : QString(data_polis.enp.trimmed().at(12)) ) ).
        replace("#14", ( (data_polis.enp.isEmpty() || data_polis.enp.trimmed().length()<14) ? QString(" ") : QString(data_polis.enp.trimmed().at(13)) ) ).
        replace("#15", ( (data_polis.enp.isEmpty() || data_polis.enp.trimmed().length()<15) ? QString(" ") : QString(data_polis.enp.trimmed().at(14)) ) ).
        replace("#16", ( (data_polis.enp.isEmpty() || data_polis.enp.trimmed().length()<16) ? QString(" ") : QString(data_polis.enp.trimmed().at(15)) ) ).

        replace("#X#", ( (data_polis.enp.trimmed().isEmpty()) ? "V" : " ") );
    ...
    s_content = s_content.
        replace("#SNILS#", data_pers.snils.replace("-","").replace(" ","")).
    ...
    if (data_pers.rep_id>0 && data_pers.rep_relation==1) {
        s_content = s_content.replace("#RR1#", "V").replace("#RR2#", " ").replace("#RR0#", " ");
    } else if (data_pers.rep_id>0 && data_pers.rep_relation==2) {
        s_content = s_content.replace("#RR1#", " ").replace("#RR2#", "V").replace("#RR0#", " ");
    } else if (data_pers.rep_id>0) {
        s_content = s_content.replace("#RR1#", " ").replace("#RR2#", " ").replace("#RR0#", "V");
    } else {
        s_content = s_content.replace("#RR1#", " ").replace("#RR2#", " ").replace("#RR0#", " ");
    }
    ...
    // архивация шаблона
    // ===========================
    QString fname_res = data_app.path_out + "_STATEMENTS_/req_ch_smo - " + data_pers.fam + " " + data_pers.im + " " + data_pers.ot + " - " + date_to_str(data_pers.date_birth) + "odt";
...
void print_ch_smo_wnd::on_bn_edit_clicked(bool checked) {
    if (checked) {
...
void print_ch_smo_wnd::on_bn_cansel_clicked() {
    reject();
}
