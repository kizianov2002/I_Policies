...
#include "ui_spr_assig_errors_wnd.h"

spr_assig_errors_wnd::spr_assig_errors_wnd(QSqlDatabase &db, s_data_app &data_app, QWidget *parent) :
    db(db), data_app(data_app), QDialog(parent), ui(new Ui::spr_assig_errors_wnd)
{
    ui->setupUi(this);
...
spr_assig_errors_wnd::~spr_assig_errors_wnd() {
    delete ui;
}

void spr_assig_errors_wnd::on_bn_close_clicked() {
    close();
}

void spr_assig_errors_wnd::on_bn_refresh_clicked() {
    refresh_errors_tab();
}

void spr_assig_errors_wnd::refresh_errors_tab() {
...
void spr_assig_errors_wnd::on_tab_errors_clicked(const QModelIndex &index) {
    if (!db.isOpen()) {
        QMessageBox::warning(this, ("Нет доступа к базе данных"), ("Нет открытого соединения к базе данных?\n Это ненормальная ситуация и вы не должны были увидеть этого сообщения..(\n Перезапустите программу. Если это сообщение будет повторяться - сообщите разработчику."));
        return;
    }
...
void spr_assig_errors_wnd::on_bn_delete_clicked() {
    if (!db.isOpen()) {
        QMessageBox::warning(this, ("Нет доступа к базе данных"), ("Нет открытого соединения к базе данных?\n Это ненормальная ситуация и вы не должны были увидеть этого сообщения..(\n Перезапустите программу. Если это сообщение будет повторяться - сообщите разработчику."));
        return;
    }
    ...
    if (indexes.size()>0) {
        ...
        QString sql_del = "delete from spr_assig_errors "
...
void spr_assig_errors_wnd::on_bn_add_clicked() {
    if (!db.isOpen()) {
        QMessageBox::warning(this, ("Нет доступа к базе данных"), ("Нет открытого соединения к базе данных?\n Это ненормальная ситуация и вы не должны были увидеть этого сообщения..(\n Перезапустите программу. Если это сообщение будет повторяться - сообщите разработчику."));
        return;
    }
    ...
    if (change_error_w->exec()) {
        ...
        QString sql_add = "insert into spr_assig_errors(code, action, text, comment) "
